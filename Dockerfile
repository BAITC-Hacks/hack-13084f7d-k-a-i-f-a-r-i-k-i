...
# Complete local MVP: Streamlit, durable analysis, evaluation and OCR.
...
RUN uv sync --locked --no-dev --extra pipeline --extra ui --no-install-project
...
COPY --chown=orgagent:orgagent .streamlit/ ./.streamlit/
RUN uv sync --locked --no-dev --extra pipeline --extra ui --no-editable
...
RUN uv sync --locked --no-dev --extra pipeline --extra ui --extra dev --no-install-project
...
COPY --chown=orgagent:orgagent ui/ ./ui/
COPY --chown=orgagent:orgagent .streamlit/ ./.streamlit/
RUN uv sync --locked --no-dev --extra pipeline --extra ui --extra dev --no-editable
...
USER orgagent
EXPOSE 8501
HEALTHCHECK --interval=15s --timeout=3s --start-period=20s --retries=4 CMD python -c "import urllib.request; urllib.request.urlopen('http://127.0.0.1:8501/_stcore/health', timeout=2)"
CMD ["streamlit", "run", "ui/app.py", "--server.address=0.0.0.0", "--server.port=8501", "--server.headless=true", "--server.fileWatcherType=none"]
